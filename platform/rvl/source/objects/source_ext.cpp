...
using DSP = love::DSP<Console::RVL>;

template<>
Source<Console::RVL>::DataBuffer::DataBuffer(const void* data, size_t size) : size(size)
{
...
template<>
Source<Console::RVL>::DataBuffer::~DataBuffer()
{
    free(this->buffer);
}

Source<Console::RVL>::Source(AudioPool* pool, SoundData* soundData) :
    Source<>(TYPE_STATIC),
    pool(pool),
    buffers {}
{
...
Source<Console::RVL>::Source(AudioPool* pool, Decoder* decoder) :
    Source<>(TYPE_STREAM),
    pool(pool),
    buffers {}
{
...
Source<Console::RVL>::Source(AudioPool* pool, int sampleRate, int bitDepth, int channels,
                              int buffers) :
    Source<>(TYPE_QUEUE),
    pool(pool),
    buffers {}
...
Source<Console::RVL>::Source(const Source& other) :
...
Source<Console::RVL>::~Source()
{
    this->Stop();
}

Source<Console::RVL>* Source<Console::RVL>::Clone()
{
    return new Source(*this);
}

bool Source<Console::RVL>::Play()
{
...
void Source<Console::RVL>::Reset()
{}

void Source<Console::RVL>::Stop()
{
    if (!this->valid)
        return;
...
bool Source<Console::RVL>::IsPlaying() const
{
    return this->valid && !::DSP::Instance().IsChannelPaused(this->channel);
}

bool Source<Console::RVL>::IsFinished() const
{
    if (!this->valid)
        return false;
...
bool Source<Console::RVL>::Update()
{
    if (!this->valid)
        return false;
...
void Source<Console::RVL>::SetVolume(float volume)
{
...
float Source<Console::RVL>::GetVolume() const
{
    if (this->valid)
...
/* todo */
void Source<Console::RVL>::Seek(double offset, Unit unit)
{
    // auto lock = this->pool->Lock();
...
/* todo */
double Source<Console::RVL>::Tell(Unit unit)
{
    auto lock = this->pool->Lock();
...
double Source<Console::RVL>::GetDuration(Unit unit)
{
    auto lock = this->pool->Lock();
...
void Source<Console::RVL>::SetLooping(bool loop)
{
...
/* todo */
bool Source<Console::RVL>::Queue(void* data, size_t length, int sampleRate, int bitDepth,
                                  int channels)
{
    if (this->sourceType != TYPE_QUEUE)
...
int Source<Console::RVL>::GetFreeBufferCount() const
{
    if (this->sourceType == TYPE_STATIC)
        return 0;
...
void Source<Console::RVL>::PrepareAtomic()
{
...
int Source<Console::RVL>::StreamAtomic(Mix_Chunk& buffer, Decoder* decoder)
{
...
/* todo */
void Source<Console::RVL>::TeardownAtomic()
{
...
bool Source<Console::RVL>::PlayAtomic(Mix_Chunk& buffer)
{
    this->PrepareAtomic();
...
void Source<Console::RVL>::StopAtomic()
{
    if (!this->valid)
        return;
...
void Source<Console::RVL>::PauseAtomic()
{
    if (this->valid)
        ::DSP::Instance().ChannelPause(this->channel);
}

void Source<Console::RVL>::ResumeAtomic()
{
    if (this->valid)
        ::DSP::Instance().ChannelPause(this->channel, false);
}

bool Source<Console::RVL>::Play(const std::vector<Source*>& sources)
{
    if (sources.size() == 0)
        return true;
...
void Source<Console::RVL>::Stop(const std::vector<Source*>& sources)
{
    if (sources.size() == 0)
        return;
...
void Source<Console::RVL>::Pause(const std::vector<Source*>& sources)
{
    if (sources.size() == 0)
        return;
...
std::vector<Source<Console::RVL>*> Source<Console::RVL>::Pause(AudioPool* pool)
{
...
void Source<Console::RVL>::Stop(AudioPool* pool)
{
    std::vector<Source*> sources;
...
int Source<Console::RVL>::GetChannelCount() const
{
