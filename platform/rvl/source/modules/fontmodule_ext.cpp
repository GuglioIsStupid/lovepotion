...
#include <objects/data/systemfont/systemfont.hpp>

using namespace love;

FontModule<Console::RVL>::FontModule() : FontModule<Console::ALL>()
{
    this->defaultFontData.Set(new SystemFont(OS_SHAREDDATATYPE_FONT_STANDARD), Acquire::NORETAIN);
}

Rasterizer* FontModule<Console::RVL>::NewImageRasterizer(ImageData<Console::Which>* data,
...
Rasterizer* FontModule<Console::RVL>::NewRasterizer(FileData* data) const
{
    if (TrueTypeRasterizer<Console::RVL>::Accepts(this->library, data))
...
Rasterizer* FontModule<Console::RVL>::NewTrueTypeRasterizer(
...
{
    return new TrueTypeRasterizer<Console::RVL>(this->library, data, size, dpiScale, hinting);
}
